...
func (s *Server) createServices() {
	s.services.user = service.NewUser(s.db, s.repos.user, s.repos.auth, s.repos.opLog)
	s.services.auth = service.NewAuth(s.db, s.repos.auth, s.repos.user, s.repos.opLog)
	s.services.wpp = service.NewWhatsApp(s.db, s.repos.opLog)
	...
	s.services.sale = service.NewSale(s.db, s.repos.sale, s.repos.sku, s.repos.user, s.repos.saleItem, s.repos.stock, s.repos.opLog)
	s.services.retrieval = service.NewRetrieval(s.db, s.repos.retrieval, s.repos.user, s.repos.saleItem, s.repos.opLog)
